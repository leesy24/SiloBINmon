...
Private InitCRC32_done As Boolean

Private OnBits(0 To 31) As Long
Private MakeOnBits_done As Boolean
...
Private Function LShiftLong(ByVal Value As Long, ByVal Shift As Integer) As Long
    If MakeOnBits_done = False Then
        MakeOnBits
        MakeOnBits_done = True
    End If
...
Public Function GetLong4Bytes(ByRef bArray() As Byte, offset As Integer) As Long
    GetLong4Bytes = LShiftLong(bArray(offset + 0), 24) _
                    Or CLng(bArray(offset + 1) * (256 ^ 2)) _
                    Or CLng(bArray(offset + 2)) * (256) _
                    Or CLng(bArray(offset + 3))
End Function
